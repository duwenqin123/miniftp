...
typedef struct ftpcmd {
	...
	void (*cmd_handler)(session_t *session1);
} ftpcmd_t;
...
/*
 * 数据连接的超时处理
 * 注意，初始时进程设置控制连接的闹钟和超时处理函数，之后，建立数据连接时，
 * 再次设置数据连接的闹钟和超时处理函数，由于一个进程只能有一个闹钟，
 * 故新的闹钟会取代控制连接的闹钟。之后，当开始传输数据前，令data->process为1。
 * 于是，若建立了数据连接却不传输数据，此时data->process为0，
 * 闹钟到达时应该断开连接。而若data->process为1，闹钟到了，重新建立闹钟，
 * 令data->process为0，这样，再次超时后就断开数据连接
 */
void handle_sigalrm(int sig)
{
...
/*
 *抓包显示，传输数据时，若客户端按下ctrl+c，则会向对方发送ff f4 ff和f2 ABOR两个包，
 * 其中前一个包设置了URG
 */
void handle_sigurg(int sig) {
    if (p_sess->data_fd == -1)
...
int list_common(session_t *sess, int detail)
{
    ...
    while ((dt = readdir(dir)) != NULL) {
        ...
        if (detail) {
			...
			const char *dates = statbuf_get_date(&sbuf);
			off += sprintf(buf + off, "%s ", dates);
...
void upload_common(session_t *sess, int is_append)
{
	...
	enum FLAG{COMPLETED,FAIL_WRITE, FAIL_READ};
...
//			flag = 1;
	...
	if (flag == COMPLETED && !sess->abor_received)
	{
		// 226
		ftp_reply(sess, FTP_TRANSFEROK, "Transfer complete.");
	}
	else if (flag == FAIL_WRITE)
	{
		// 451
		ftp_reply(sess, FTP_BADSENDFILE, "Failure writting to local file.");
	}
	else if (flag == FAIL_READ)
	{
		// 426
...
/*
 * 主要逻辑是，通过child_fd发送PRIV_SOCK_GET_DATA_SOCK，之后nobody进程收到该命令后，
 * 调用privop_pasv_get_data_sock，该函数通过parent_fd接收本函数发送的客户端的ip和port，
 * 从而创建数据连接。再把新创建的fd发送回本函数
 */
int get_port_fd(session_t *sess)
{
	/*
